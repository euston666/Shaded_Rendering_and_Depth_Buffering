 ...
 * 30 January 2008
 * 
 * - translated from C code by Stan Sclaroff from CS480 2006 assignment 2, "quaternion.c"
 * 
...
public class Quaternion {
	...
	/**
	 * Returns a new quaternion representing the product of this and the specified other quaternion.
	 * 
	 * @param that
	...
	public Quaternion multiply(final Quaternion that) {
		...
		// s = s1*s2 - v1.v2
		final float newS = this.s * that.s - this.v.x * that.v.x - this.v.y * that.v.y - this.v.z * that.v.z;

		// v = s1 v2 + s2 v1 + v1 x v2
		float i = (this.s * that.v.x) + (that.s * this.v.x) + (this.v.y * that.v.z - this.v.z * that.v.y);
		float j = (this.s * that.v.y) + (that.s * this.v.y) + (this.v.z * that.v.x - this.v.x * that.v.z);
		float k = (this.s * that.v.z) + (that.s * this.v.z) + (this.v.x * that.v.y - this.v.y * that.v.x);
	...
	public Vector get_v() {
		Vector res = new Vector(this.v);
		return (res);
	}

	/*
	 * Returns a 4 by 4 matrix which represents a transformation equivalent to that of this quaternion.
	 * 
	...
	public String toString() {
		return "(" + this.s + ", " + this.v.x + ", " + this.v.y + ", " + this.v.z + ")";
	}
}
